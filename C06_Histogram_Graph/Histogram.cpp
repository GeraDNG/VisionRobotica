...
void openfiles ()
{
     ...
     printf(" Enter name of the Excel output histogram file *.xls (example: lenaout.xls) ");
     scanf("%s",&outfile);
...
//----------------------------------------------------------------------------//
//            The output file must be opened in Excel						  //
...
void userdefined ()
{
	...
	unsigned int histogramCount[256];
	// Variable to read the current pixel:
	unsigned char Pixel;
	// Counter variable:
	int i;
	// Char variable, for string convertion used below:
	char string[10];
	...
	//Pixel counting
	// Read the first Pixel from input image:
	Pixel = fgetc(infptr);
	// Loop through all Pixel in input image:
	do{
		histogramCount[Pixel]++;
		Pixel = fgetc(infptr);
	}   while(!feof(infptr));
	
	// Convert histogramCount integer vector to string entries with a -
	// carriage return at the end that will be used as delimiter for Excel
	for ( i=0; i<256; i++  )
	{
		itoa(histogramCount[i],string,10);    // itoa = converts integer to ASCII
		fputs(string,outfptrH);     		  // put string in the file
		fputc('\n',outfptrH);				  // writing an Enter
	}
